...
SELECT
  ...
  ROUND(SUM(combined_total_item_value) OVER(PARTITION BY customer_state), 2) AS saler_per_state,
  ROUND((combined_total_item_value / SUM(combined_total_item_value) OVER(PARTITION BY customer_state)) * 100, 2) AS pay_type_proportion
